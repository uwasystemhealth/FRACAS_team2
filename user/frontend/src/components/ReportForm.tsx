...
import OwnerMenu, { Owner } from "@/components/ViewReportComponents/OwnerMenu";
...
const ReportForm: React.FC = (props: Props) => {
    ...
    API_CLIENT.get(API_ENDPOINT.USER + `/current`)
      .then((response) => {
        if (response.status == 200) {
          ...
          // });
          reset({
            team_id: response.data.team_id,d
          });
        } else {
  ...
  const handleOwnerorTeamChange = (event: SelectChangeEvent) => {
    var team_id_value=event.target.value
    setTeam_id(team_id_value);
    reset({
                  ...
                  <Controller
                    ...
                    render={({ field }) => (
                      <Select
                        ...
                        // value={field.value}
                        value={team_id}
                        onChange={handleOwnerorTeamChange}
                        fullWidth
                      >
                        {teams.map((team) => (
                          <MenuItem key={team.id} value={team.id}>{team.leader_name}</MenuItem>
                        ))}
                      </Select>
                    )}
                  />
